...
export async function parseWordDocument(file) {
  try {
    ...
    // Step 3: Detect market and project name
...
function detectMarket(filename) {
  // Match pattern: ProjectName_Marketing_Copy_en-US_2025-10-15.docx
  // Or legacy: Marketing_Copy_en-US.docx
  const marketMatch = filename.match(/_([a-z]{2}-[A-Z]{2})(?:_\d{4}-\d{2}-\d{2})?\.docx$/i);
  return marketMatch ? marketMatch[1] : 'unknown';
}

// Helper: Extract project name from filename
function extractProjectName(filename) {
  // Match pattern: ProjectName_Marketing_Copy_en-US_2025-10-15.docx
  // Extract everything before "_Marketing_Copy"
  const projectMatch = filename.match(/^(.+?)_Marketing_Copy_/i);
  if (projectMatch) {
    return projectMatch[1];
  }

  // Fallback: try to extract from legacy format or use generic name
  const baseMatch = filename.match(/^(.+?)_[a-z]{2}-[A-Z]{2}/i);
  if (baseMatch) {
    return baseMatch[1];
  }
